...
FCOS에는 선택적으로 MissBank 기반 positive loss weighting이 추가된다. 현재 ReMiss 런타임은 memory와 replay 중심 경로이며, detector feature를 직접 주입하지 않는다.
...
| FCOS loss weighting | `models/detection/wrapper/fcos.py` | `loss_weight.enabled: true`일 때만 FCOS train loss path 변경 |
...
FCOS에서 `loss_weight.enabled: true`이면 MissBank가 GT별 weight를 만든다. 반복 miss된 GT일수록 positive classification, box regression, centerness loss weight가 커질 수 있다.

이 기능은 FCOS 학습에만 적용된다.

| 모델 | 적용 |
|---|---|
| FCOS | classification, bbox regression, centerness positive loss weighting |
| Faster R-CNN | 적용 안 함 |
| DINO | 적용 안 함 |
...
이 문서의 범위는 현재 연결된 memory, optional loss weighting, replay sampler에 한정한다. 새로운 auxiliary head, feature injection, 실패 유형별 replay 같은 기능을 도입하려면 별도 코드 경로, 설정, 출력 문서가 함께 추가되어야 한다.
