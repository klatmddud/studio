...
현재 runtime은 MissBank 안정성 지표를 저장하지 않는다. FTMB는 `modules/cfg/ftmb.yaml`에서 독립적으로 켜고 끄며, localization, classification, both, missed, duplicate, background failure type별 개수를 저장한다.
