...
구현된 FCDR v1은 `HardReplayPlanner.build_epoch_index(...)`에서 image-level replay index와 crop-level replay index를 함께 만든다.
...
4. `ReplayIndex.replay_crops`에 실제 `ReplayCrop` 또는 확장 spec을 채운다.

`ReplayCrop`은 `dataset_index`, `failure_type`, `mode`, `crop_box_abs`, `source_bbox_abs`, `support_box_abs`, `sampling_weight`, `replay_cap`을 포함한다. Sampler는 base dataset 길이 이후의 virtual index를 crop replay sample로 사용한다.
...
```python
class CounterfactualReplayDataset(Dataset):
    ...
    def __getitem__(self, index):
        if index >= len(self.base_dataset):
            return self._build_replay_crop_sample(index)
        return self.base_dataset[index]
```

초기 구현은 crop replay부터 시작한다.
...
6. 원본 id와 replay metadata는 `source_image_id`, `replay_gt_uid`, `replay_failure_type`, `replay_mode`로 보존한다.
7. `is_replay: true`를 설정해 post-step MDMB/MDMB++/FAR memory update에서 제외한다.

FCDR replay index는 epoch마다 바뀐다. worker process가 stale replay index를 들고 있지 않도록 `CounterfactualReplayDataset` 사용 시 persistent workers는 비활성화한다.

### 9.3 Copy-Paste Replay

copy-paste는 두 번째 단계로 구현한다.

1. source object crop을 만든다.
2. target base image를 샘플링한다.
3. overlap constraint를 만족하는 위치에 paste한다.
4. target annotations에 pasted box를 추가한다.

첫 버전에서는 segmentation mask가 없으므로 rectangular paste를 사용한다. mask가 없는 COCO detection setting에서는 rectangle paste artifact가 생길 수 있으므로 paste probability를 낮게 시작한다.
...
- 같은 mini-batch 보장은 v1에서 필수로 하지 않는다.
- RASD와 결합할 때만 same mini-batch pair constraint를 고려한다.

## 10. Config Proposal

현재 구현은 `modules/cfg/hard_replay.yaml`의 nested `fcdr` 섹션을 사용한다. FCDR은 top-level Hard Replay `enabled: true`일 때만 활성화된다.
...
| `copy_paste_prob` | 0.0 | v1에서는 copy-paste 비활성 |
| `pair_replay_prob` | 0.0 | v1에서는 strict pair replay 비활성 |
...
1. `HardReplayPlanner`가 `ReplayIndex.replay_crops`를 채운다.
2. `CounterfactualReplayDataset`이 crop replay sample을 생성한다.
...
4. copy-paste는 비활성 기본값으로 두고 crop replay가 안정화된 뒤 켠다.
