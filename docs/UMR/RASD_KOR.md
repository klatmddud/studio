...
기본 loss는 support attraction 방식이며, `confuser.enabled: true`이면 class-confusion relapse
target에 한해 confuser-aware contrastive term을 추가한다.
...
confuser가 켜진 경우에는 MDMB++가 저장한 wrong-class candidate feature를 현재 FPN에서 다시
pooling하고, support feature를 positive, confuser candidate feature를 negative로 보는 softmax
contrastive loss를 더한다.
...
- `cls_confusion` entry이고 confuser가 켜져 있으면 wrong-class candidate를 negative로 사용할 수 있다.
...
confuser target은 `entry.failure_type == "cls_confusion"`인 경우에만 고른다.
후보는 `MDMBPlusEntry.topk_candidates`에서 가져오며 다음 조건을 만족해야 한다.

- candidate label이 GT class와 다름
- `candidate.iou_to_gt >= confuser.iou_threshold`
- `candidate.score >= confuser.min_score`
- score, IoU, survived-selection 순으로 정렬했을 때 `max_candidates_per_gt` 안에 들어감
...
feature와 cosine attraction loss를 계산한다. confuser가 켜져 있으면 선택된 wrong-class
candidate box에서도 현재 FPN feature를 pooling하고, 이 negative feature는 detach해서
current GT feature 쪽으로만 gradient가 흐르게 한다.
...
cls_confusion confuser가 있으면 contrastive loss 계산
losses["rasd"] 추가
```
...
- `confuser_targets`: wrong-class confuser negative가 하나 이상 선택된 target 수
- `mean_contrastive_loss`: confuser target 기준 contrastive loss 평균
...
- `confuser_targets`가 0이면 confuser 조건을 만족한 `cls_confusion` relapse가 없거나 후보 필터가 너무 강한 것이다.
...
`confuser.enabled: true`이면 class-confusion relapse를 더 직접적으로 다루는 negative contrastive
constraint가 적용된다. 성능이 낮아지면 먼저 `alpha_contrastive`, `confuser.iou_threshold`,
`confuser.min_score`, `max_candidates_per_gt`를 낮거나 보수적인 값으로 조정한다.
